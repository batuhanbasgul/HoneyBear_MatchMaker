...
class BeingLikedAdapter(
        private val currentUserId:String,
        private val mContext: Context,
        private val userList: ArrayList<User>,
        private val likedUserList: ArrayList<User>
        ) : RecyclerView.Adapter<BeingLikedAdapter.CardViewHolder>() {
    ...
    inner class CardViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun bindUI(user: User){
            ...
            if(user.userImageUrl != "no_image"){
                Glide.with(imageViewCardView).load(user.userImageUrl).placeholder(R.drawable.ic_profile_64).into(imageViewCardView)
            }
